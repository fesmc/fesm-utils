...
def build_utils(machine, compiler, omp, debug):
    ...
    # If utils/Makefile already exists (e.g. generated by `configme`), reuse it
    # and just build -- do not regenerate it from the legacy flat config. This
    # lets configme own the utils build configuration (central machine/compiler
    # fragments + auto-detected netCDF). Standalone builds without configme fall
    # through to the legacy `config.py` path below.
    makefile = ROOT / "utils" / "Makefile"
    if makefile.exists():
        print("  utils: using existing Makefile (skipping config.py)")
        cmd = (
    ...
    if cfg is None:
        die(
            ...
            f"reference it in [utils.config] (or generate utils/Makefile with "
            f"`configme`)."
        )
