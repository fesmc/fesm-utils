...
!     use mod_toms526
    ...
    type coord_system
        ! Coordinate-reference metadata shared by points, grids and maps.
        ! Defined once here and embedded as %cs in each geometry type.
        character(len=256)     :: mtype     ! latlon, cartesian, stereographic, ..., gaussian
        character(len=256)     :: units     ! units of the axes
        type(planet_class)     :: planet    ! which planet are we on?
        type(projection_class) :: proj      ! projection parameters
        real(dp)               :: xy_conv   ! axis-units -> meters conversion factor
        logical                :: is_cartesian, is_projection, is_lon180
    end type

    type points_class

        character(len=256) :: name      ! name of this domain (world, GRL, etc)
        integer            :: npts
        type(coord_system) :: cs        ! coordinate-system metadata
    ...
    type grid_class
        ...
        integer            :: npts
        type(coord_system) :: cs        ! coordinate-system metadata
        type(grid_axis_class) :: G      ! grid axes information
    ...
    subroutine grid_area(grid)
        ...
        do j = 1, grid%G%ny 
            do i = 1, grid%G%nx 
                ...
                vertx = [x1,x1,x2,x2] * grid%cs%xy_conv 
                verty = [y1,y2,y2,y1] * grid%cs%xy_conv 
                
                if (.not. grid%cs%is_projection .and. grid%cs%is_cartesian) then 
                    ! Calculate cartesian area
                    grid%area(i,j) = cartesian_area(vertx,verty)

                else if (grid%cs%is_projection) then 
                    ! Calculate projected area 

                    do q = 1, 4
!                         call inverse_oblique_sg_projection(vertx(q),verty(q), &
!                                                            vertlon(q),vertlat(q),grid%cs%proj)
                        call oblimap_projection_inverse(vertx(q),verty(q), &
                                                        vertlon(q),vertlat(q),grid%cs%proj)
                    end do 

                    ! Calculate latlon area
                    grid%area(i,j) = planet_area(grid%cs%planet%a,grid%cs%planet%f,vertlon,vertlat)

                else 
                    ! Calculate latlon area 
                    grid%area(i,j) = planet_area(grid%cs%planet%a,grid%cs%planet%f,vertx,verty)
        ...
        if (.not. grid%cs%xy_conv .eq. 0.d0) grid%area = grid%area / (grid%cs%xy_conv*grid%cs%xy_conv) 
    ...
    subroutine grid_init_from_grid(grid,grid0,name,x,y,x0,dx,nx,y0,dy,ny)
        ...
        call grid_init_from_opts(grid,name,mtype=grid0%cs%mtype,units=grid0%cs%units, &
    ...
    subroutine grid_init_from_points(grid,pts0,name,x,y,x0,dx,nx,y0,dy,ny)
        ...
        call grid_init_from_opts(grid,name,mtype=pts0%cs%mtype,units=pts0%cs%units, &
    ...
    subroutine grid_init_from_opts(grid,name,mtype,units,planet,lon180, &
        ...
        select case(trim(grid%cs%mtype))
            ...
            case("latitude_longitude","latlon","gaussian")
                ...
                tmp1 = planet_distance(grid%cs%planet%a,grid%cs%planet%f,grid%G%x(1),grid%G%y(1),grid%G%x(2),grid%G%y(1))
                tmp2 = planet_distance(grid%cs%planet%a,grid%cs%planet%f,grid%G%x(1),grid%G%y(1),grid%G%x(grid%G%nx),grid%G%y(1))
                ...
                tmp1 = planet_distance(grid%cs%planet%a,grid%cs%planet%f,grid%G%x(1),grid%G%y(1),grid%G%x(1),grid%G%y(2))
                tmp2 = planet_distance(grid%cs%planet%a,grid%cs%planet%f,grid%G%x(1),grid%G%y(1),grid%G%x(1),-90.0_dp)
                if (tmp2 .le. tmp1*2.0_dp) grid%border(:,1)         = 0

                ! Next check upper latitude
                tmp1 = planet_distance(grid%cs%planet%a,grid%cs%planet%f,grid%G%x(1),grid%G%y(grid%G%ny),grid%G%x(1),grid%G%y(grid%G%ny-1))
                tmp2 = planet_distance(grid%cs%planet%a,grid%cs%planet%f,grid%G%x(1),grid%G%y(grid%G%ny),grid%G%x(1),90.0_dp)
        ...
        if ( .not. grid%cs%is_cartesian ) then 
            if ( grid%cs%is_lon180 ) &  ! Then make range -180=>180
        ...
        if ( grid%cs%is_projection ) then 
            tmp1 = optimal_alpha(grid%cs%planet%R,grid%G%nx,grid%G%ny, &
                                  grid%G%dx*grid%cs%xy_conv,grid%G%dy*grid%cs%xy_conv)
    ...
    subroutine points_init_from_points(pts,pts0,name,filename,x,y,dx,dy,latlon,skip)
        ...
        if (present(filename)) then 
            call points_init_from_file(pts,name,mtype=pts0%cs%mtype,units=pts0%cs%units, &
                                       filename=trim(filename),planet=pts0%cs%planet%name, &
            ...
            call points_init_from_opts(pts,name,mtype=pts0%cs%mtype,units=pts0%cs%units, &
    ...
    subroutine points_init_from_grid(pts,grid0,name,filename,x,y,dx,dy,latlon,skip)
        ...
        if (present(filename)) then 
            call points_init_from_file(pts,name,mtype=grid0%cs%mtype,units=grid0%cs%units, &
                                       filename=trim(filename),planet=grid0%cs%planet%name, &
            ...
            call points_init_from_opts(pts,name,mtype=grid0%cs%mtype,units=grid0%cs%units, &
    ...
    subroutine points_init_from_opts(pts,name,mtype,units,planet,lon180,x,y,dx,dy, &
        ...
        pts%name  = trim(name)
        pts%cs%mtype = trim(mtype)
        pts%cs%units = trim(units)
        
        verbose_in = .TRUE. 
        if (present(verbose)) verbose_in = verbose 

        ! Also define the standard cf name
        select case(trim(pts%cs%mtype))
            case("latitude_longitude","latlon","gaussian")
        ...
        select case(trim(pts%cs%units))
            case("kilometers","km")
                pts%cs%xy_conv = 1.d3  ! To convert from km => m (for cartesian grids)
            case DEFAULT
                pts%cs%xy_conv = 1.d0 
        end select

        ! Check latlon range (0=>360 or -180=>180)
        pts%cs%is_lon180 = .FALSE.
        if (present(lon180)) pts%cs%is_lon180 = lon180 
        ...
        call planet_init(pts%cs%planet,planet_name)
        ...
        if ( pts%cs%is_projection ) then
            ...
            if (latlon_in) then 
                ...
                do i = 1, pts%npts       
                    call oblimap_projection(pts%lon(i),pts%lat(i),pts%x(i),pts%y(i),pts%cs%proj)
                    pts%x(i) = pts%x(i)/pts%cs%xy_conv
                    pts%y(i) = pts%y(i)/pts%cs%xy_conv
                end do
                ...
                do i = 1, pts%npts       
                    call oblimap_projection_inverse(pts%x(i)*pts%cs%xy_conv,pts%y(i)*pts%cs%xy_conv, &
                                                    pts%lon(i),pts%lat(i),pts%cs%proj)
                end do

            end if 

        else if ( .not. pts%cs%is_cartesian ) then 
            ...
            call projection_init(pts%cs%proj,"Undefined",pts%cs%planet)

        end if 

        ! Make sure final lon/lat values are in desired range
        ! (default range is 0=>360)
        if ( pts%cs%is_projection .or. .not. pts%cs%is_cartesian ) then 
            if ( pts%cs%is_lon180 ) &  ! Then make range -180=>180
    ...
    subroutine grid_to_points(grid,pts,mask_pack,define)
        ...
        if (define_fields) then 
            ...
            pts%cs%mtype         = trim(grid%cs%mtype)
            pts%cs%units         = trim(grid%cs%units)
            pts%npts          = grid%npts
            pts%cs%is_cartesian  = grid%cs%is_cartesian 
            pts%cs%is_projection = grid%cs%is_projection 
            pts%cs%is_lon180     = grid%cs%is_lon180
            pts%cs%planet        = grid%cs%planet
            pts%cs%proj          = grid%cs%proj
            pts%cs%xy_conv       = grid%cs%xy_conv 
        end if 
    ...
    subroutine points_to_grid(pts,grid,mask_pack,define)
        ...
        grid%cs%mtype         = trim(pts%cs%mtype)
        grid%cs%units         = trim(pts%cs%units)
        grid%npts          = pts%npts
        grid%cs%is_cartesian  = pts%cs%is_cartesian 
        grid%cs%is_projection = pts%cs%is_projection 
        grid%cs%is_lon180     = pts%cs%is_lon180
        grid%cs%planet        = pts%cs%planet 
        grid%cs%proj          = pts%cs%proj
        grid%cs%xy_conv       = pts%cs%xy_conv 
    ...
    function compare_coord_points_points(pts1,pts2) result(same_map)
        ...
        if (pts1%cs%is_projection .and. pts2%cs%is_projection        .and. &
            trim(pts1%cs%mtype)       .eq. trim(pts2%cs%mtype)       .and. &
            same_projection(pts1%cs%proj,pts2%cs%proj) ) then 
            
            ! Both maps come from the same projection
            same_map = .TRUE. 

        else if (pts1%cs%is_cartesian .and. .not. pts1%cs%is_projection .and. &
                 pts2%cs%is_cartesian .and. .not. pts2%cs%is_projection) then 
            ! Both maps are generic cartesian grids (assume origin is the same)
            same_map = .TRUE. 

        else if (.not. pts1%cs%is_cartesian .and. .not. pts2%cs%is_cartesian) then 
    ...
    function compare_coord_grid_grid(grid1,grid2) result(same_map)
        ...
        ! Check if both maps use the same projection  
        if (grid1%cs%is_projection .and. grid2%cs%is_projection        .and. &
            trim(grid1%cs%mtype)       .eq. trim(grid2%cs%mtype)       .and. &
            same_projection(grid1%cs%proj,grid2%cs%proj) ) then 
            
            ! Both maps come from the same projection
            same_map = .TRUE. 

        else if (grid1%cs%is_cartesian .and. .not. grid1%cs%is_projection .and. &
                 grid2%cs%is_cartesian .and. .not. grid2%cs%is_projection) then 
            ! Both maps are generic cartesian grids (assume origin is the same)
            same_map = .TRUE. 

        else if (.not. grid1%cs%is_cartesian .and. .not. grid2%cs%is_cartesian) then 
            ! Both maps are latlon maps
            same_map = .TRUE. 
    ...
    subroutine grid_write(grid,fnm,xnm,ynm,create)
        ...
        if (trim(grid%cs%units) .eq. "degrees") then 
            xunits = "degrees_east"
            yunits = "degrees_north"
        else 
            xunits = trim(grid%cs%units)
            yunits = trim(grid%cs%units)
        end if 
        ...
        if (grid%cs%is_projection .or. grid%cs%is_cartesian) then 
            ...
            call nc_write_attr(fnm,"x2D","units",grid%cs%units)
            call nc_write(fnm,"y2D",grid%y,dim1=xnm,dim2=ynm,grid_mapping="crs")
            call nc_write_attr(fnm,"y2D","units",grid%cs%units)
    ...
    subroutine points_write(pts,fnm,xnm,ynm,create)
        ...
        if (trim(pts%cs%units) .eq. "degrees") then 
            xunits = "degrees_east"
            yunits = "degrees_north"
        else 
            xunits = trim(pts%cs%units)
            yunits = trim(pts%cs%units)
        end if 
        ...
        if (pts%cs%is_projection .or. pts%cs%is_cartesian) then 
            call nc_write(fnm,xnm,pts%x,dim1="point",grid_mapping=pts%cs%mtype)
            call nc_write_attr(fnm,xnm,"units",xunits)
            call nc_write(fnm,ynm,pts%y,dim1="point",grid_mapping=pts%cs%mtype)
        ...
        if ( pts%cs%is_projection .or. (.not. pts%cs%is_cartesian) ) then  
            call nc_write(fnm,"lon",pts%lon,dim1="point",grid_mapping=pts%cs%mtype)
            !call nc_write_attr(fnm,"lon","_CoordinateAxisType","Lon")

            call nc_write(fnm,"lat",pts%lat,dim1="point",grid_mapping=pts%cs%mtype)
    ...
    subroutine points_print(pts)
        ...
        write(*,"(a24,a)") "Map type: ",trim(pts%cs%mtype)
        write(*,"(a24,a)") "Coordinate units: ",trim(pts%cs%units)
        write(*,"(a16,i8)")           "Total points = ",pts%npts
        if (pts%cs%is_cartesian) then 
...
!             write(*,*) "Projection information"
            write(*,"(a16,a)")         "proj method = ", trim(pts%cs%proj%method)
            write(*,"(a16,g12.5)")     "          a = ", pts%cs%proj%a
            write(*,"(a16,g12.5)")     "          f = ", pts%cs%proj%f
            write(*,"(a16,g12.5)")     "     lambda = ", pts%cs%proj%lambda
            write(*,"(a16,g12.5)")     "        phi = ", pts%cs%proj%phi
            write(*,"(a16,g12.5)")     "      alpha = ", pts%cs%proj%alpha
            write(*,"(a16,g12.5)")     "        x_e = ", pts%cs%proj%x_e
            write(*,"(a16,g12.5)")     "        y_n = ", pts%cs%proj%y_n
        end if 
...
!         write(*,*)
    ...
    function pts_which_nearest(pts,x,y,latlon) result(idx)
        ...
        if (is_latlon) then 
            ...
            do i = 1, pts%npts
                dist(i) = planet_distance(pts%cs%planet%a,pts%cs%planet%f,x,y,pts%lon(i),pts%lat(i))
            end do 

        else
            ! Calculate distances using cartesian distances (in meters)
            do i = 1, pts%npts
                dist(i) = cartesian_distance(x,y,pts%x(i)*pts%cs%xy_conv,pts%y(i)*pts%cs%xy_conv)
            end do 
